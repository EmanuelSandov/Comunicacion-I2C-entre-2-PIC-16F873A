// Configuraci?n de bits de configuraci?n para PIC16F873A
...
void I2C_Master_Write(unsigned d) {
    I2C_Master_Wait();
    SSPBUF = d;  // Escribe el dato en el buffer
}

unsigned char I2C_Master_Read(unsigned char ack) {
    I2C_Master_Wait();        // Asegura que el bus est? listo para la siguiente operaci?n
    RCEN = 1;                 // Habilita la recepci?n de datos en el maestro
    I2C_Master_Wait();        // Espera a que se complete la recepci?n de datos

    unsigned char data = SSPBUF;  // Lee el buffer de datos recibidos
    I2C_Master_Wait();        // Asegura que no hay operaciones pendientes antes de enviar ACK/NACK

    if (ack) {
        ACKDT = 0;            // Prepara para enviar ACK (continuar recibiendo datos)
    } else {
        ACKDT = 1;            // Prepara para enviar NACK (no m?s datos requeridos)
    }
    ACKEN = 1;                // Inicia la secuencia de ACK o NACK
